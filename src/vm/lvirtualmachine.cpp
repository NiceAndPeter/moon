...
#include "ljumptab.h"
  ...
  ** 'c_val' is the limit of live values in the stack (typically L->top or ci->top)
  **
  ...
  */
  auto checkGC = [&](lua_State* L_arg, StkId c_val) {
    luaC_condGC(L_arg,
                [&](){ saveProgramCounter(ci); L_arg->getStackSubsystem().setTopPtr(c_val); },
                [&](){ updateTrap(ci); });
